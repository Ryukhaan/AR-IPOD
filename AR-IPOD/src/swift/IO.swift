...
struct IO {
    ...
    struct Export {
        static func exportToPLY(volume: Model, at: String) -> URL {
            let size = volume.dimension * volume.dimension * volume.dimension
            let sdfs = volume.voxels.map { $0.sdf }
            var v = [Vector(0,0,0)]
            //if let dir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
            if let path = NSURL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(at) {
                //let path = dir.appendingPathComponent(at)
                let cFileName = path.absoluteString.cString(using: .utf8)
        ...
        static func toPLYFormat(mesh: [Vector], at: String) -> URL? {
        ...
        static func writePLYFormat(mesh: [Vector]) -> String {
            var res = ""
            res = "\(res)ply\n"
            res = "\(res)format ascii 1.0\n"
            res = "\(res)element vertex \(mesh.count)\n"
            res = "\(res)property float x\n"
            res = "\(res)property float y\n"
            res = "\(res)property float z\n"
            res = "\(res)element face \(mesh.count / 3)\n"
            res = "\(res)property list uchar int vertex_index\n"
            res = "\(res)end_header\n"
            ...
            // Write triangles
            for i in 0..<(mesh.count/3) {
                res = "\(res)3 \(3*i+2) \(3*i+1) \(3*i)\n"
            }
            return res
        }
        
        static func save(model: Model, atTime: Int) {
